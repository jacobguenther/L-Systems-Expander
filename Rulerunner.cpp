...
if ( rulestates.size() >= maxdepth || localscale*turtles.top().getscale() < minscale)
   ...
   switch(therules[rr].drawmethod) 
      { //!!!Use a factory here!
      case Rule::NORM:  agraphic.reset(new Linegraphic(temp)); break;
      case Rule::MIDPT: agraphic.reset(new Dropgraphic(temp,0,.5)); break;
      case Rule::INVIS: agraphic.reset(new Invisgraphic); break;
      case Rule::RECT:  agraphic.reset(new Linegraphic(temp)); break; //!!! 
      case Rule::DROP:  
...
{
while(!rulestates.empty() && !agraphic.get())
   {//Go until we hit a turtle forward, or we're done
